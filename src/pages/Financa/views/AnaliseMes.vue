<template>
  <TemplateDefault title="Análise mês">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        time: false,
        th: ['Origem','Total','mês 1','mês 2','mês 3','mês 4','mês 5','mês 6','mês 7','mês 8','mês 9','mês 10','mês 11','mês 12'],
        FIGP: [],
      ...
      getDados() {
        setTimeout( () => {
          ...
          .then( ({STATUS, data, msg}) => {
            if(STATUS == 'success'){
              this.dados = data.F_analiseMes

              const {RECEITA, th} = data.F_analiseMes
              this.th = th
